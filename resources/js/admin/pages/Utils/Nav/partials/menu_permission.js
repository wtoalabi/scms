    ...
    methods: {
        ...
        * and when checking, the two would be equal, and return false. */
        ...
        groupPermission(permits) {
            if (this.$store.state.admin.role.permissions) {
                let permissions = this.$store.state.admin.role.permissions;
                let difference = _.difference(permissions, permits);
                return difference.length < permissions.length;
            }
        },
        userCan(permission) {
            let allPermissions = this.$store.state.admin.role.permissions;
            if (allPermissions) {
