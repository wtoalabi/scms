<?php
  ...
  class Policies
  {
    ...
    private static function generatePolicyStubs($data) {
      $modelName = "{$data['namespace']}\\{$data['pluralizedName']}\Policies";
    ...
    private static function stub(string $destination, $data) {
      ...
      $policyContent = str_replace(['Dummy', 'Dummies', 'DUMMY','Path','Main', 'dummy', 'DummyModel'],
        [$data['name'], $data['pluralizedName'], Str::upper($data['name']), $data['namespace'],$data['base'], Str::lower($data['name']),$dummyModel], $dummyContent
      );
